...
for(i in 1:10){
  ...
  #Non-treated
  XD_training_sample_nt <- cbind(X_training_sample[which(D_training_sample==0),],D_training_sample[which(D_training_sample==0)])
  Y_training_sample_nt <- Y_training_sample[which(D_training_sample==0)]
  #Treated, X_1 = 1
  XD_training_sample_t1 <- cbind(X_training_sample[which((D_training_sample==1)&(X_training_sample[,1]==1)),],D_training_sample[which((D_training_sample==1)&(X_training_sample[,1]==1))])
  Y_training_sample_t1 <- Y_training_sample[which((D_training_sample==1)&(X_training_sample[,1]==1))]
  #Treated, X_1 = 0
  XD_training_sample_t0 <- cbind(X_training_sample[which((D_training_sample==1)&(X_training_sample[,1]==0)),],D_training_sample[which((D_training_sample==1)&(X_training_sample[,1]==0))])
  Y_training_sample_t0 <- Y_training_sample[which((D_training_sample==1)&(X_training_sample[,1]==0))]
  
  test_units <- sort(folds[[i]])
  X_test_sample <- X[test_units,]
  D_test_sample <- treated_1[test_units]
  #Non-treated
  XD_test_sample_nt <- cbind(X_test_sample[which(D_test_sample==0),],D_test_sample[which(D_test_sample==0)])
  #Treated, X_1 = 1
  XD_test_sample_t1 <- cbind(X_test_sample[which((D_test_sample==1)&(X_test_sample[,1]==1)),],D_test_sample[which((D_test_sample==1)&(X_test_sample[,1]==1))])
  #Treated, X_1 = 0
  XD_test_sample_t0 <- cbind(X_test_sample[which((D_test_sample==1)&(X_test_sample[,1]==0)),],D_test_sample[which((D_test_sample==1)&(X_test_sample[,1]==0))])
  
  
  #### Elastic-Net ####
  #Split by treatment status and heterogeneous group
  #Non-treated
  if(nrow(Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which(D_test_sample==0)]),])!=0){ #Checks if respective units are in test-fold and if computations are therefore necesarry
    
    EN_fit <- cv.glmnet(as.matrix(XD_training_sample_nt), Y_training_sample_nt, type.measure = 'mse', alpha = .5)
    Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which(D_test_sample==0)]),'Elastic Net'] <- predict(EN_fit, s = EN_fit$lambda.1se, newx = as.matrix(XD_test_sample_nt))
    
  }
  
  #Treated, X_1 = 1
  if(nrow(Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==1))]),])!=0){
    
    EN_fit <- cv.glmnet(as.matrix(XD_training_sample_t1), Y_training_sample_t1, type.measure = 'mse', alpha = .5)
    Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==1))]),'Elastic Net'] <- predict(EN_fit, s = EN_fit$lambda.1se, newx = as.matrix(XD_test_sample_t1))
  
  }
  
  #Treated, X_1 = 0
  if(nrow(Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==0))]),])!=0){
    
    EN_fit <- cv.glmnet(as.matrix(XD_training_sample_t0), Y_training_sample_t0, type.measure = 'mse', alpha = .5)
    Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==0))]),'Elastic Net'] <- predict(EN_fit, s = EN_fit$lambda.1se, newx = as.matrix(XD_test_sample_t0))
  ...
  #### KRLS ####
  #Non-treated
  if(nrow(Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which(D_test_sample==0)]),])!=0){
    
    non_constant <- which(!apply(XD_training_sample_nt[,base_variables], MARGIN = 2, function(x) max(x, na.rm = TRUE) == min(x, na.rm = TRUE)))
    KRLS_fit <- krls(X = XD_training_sample_nt[,base_variables][,non_constant], y = Y_training_sample_nt, derivative = FALSE)
    Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which(D_test_sample==0)]),'KRLS'] <- predict(KRLS_fit, newdata = XD_test_sample_nt[,base_variables][,non_constant])$fit
  ...
  if(nrow(Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==1))]),])!=0){
    ...
    non_constant <- which(!apply(XD_training_sample_t1[,base_variables], MARGIN = 2, function(x) max(x, na.rm = TRUE) == min(x, na.rm = TRUE)))
    KRLS_fit <- krls(X = XD_training_sample_t1[,base_variables][,non_constant], y = Y_training_sample_t1, derivative = FALSE)
    Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==1))]),'KRLS'] <- predict(KRLS_fit, newdata = XD_test_sample_t1[,base_variables][,non_constant])$fit
  ...
  if(nrow(Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==0))]),])!=0){
    ...
    non_constant <- which(!apply(XD_training_sample_t0[,base_variables], MARGIN = 2, function(x) max(x, na.rm = TRUE) == min(x, na.rm = TRUE)))
    KRLS_fit <- krls(X = XD_training_sample_t0[,base_variables][,non_constant], y = Y_training_sample_t0, derivative = FALSE)
    Y_1_hat[which(rownames(Y_1_hat) %in% test_units[which((D_test_sample==1)&(X_test_sample[,1]==0))]),'KRLS'] <- predict(KRLS_fit, newdata = XD_test_sample_t0[,base_variables][,non_constant])$fit
  ...
  SVM_fit <- SMO(Y ~ ., data = data.frame(Y = Y_training_sample, X_training_sample), control = Weka_control(M = TRUE ) )
  predict(SVM_fit, newdata = data.frame(Xtfull), type="probability" )[,2] 
